...
import * as EntitySchemaRegistry from "../../orm/entity/schema-registry";
 ...
 * @returns
 */
const has_new_tables = (
  existing_schema: SchemaRegistryData,
  updated_schema: SchemaRegistryData
) => {
 ...
 * @returns
 */
const generate_create_table_query = (
  table_name: string,
  updated_schema: SchemaRegistryData
) => {
  const updated_schema_table_data = updated_schema.filter(
    (item) => item.collection === table_name
  )[0];
  ...
  for (let i = 0; i < updated_schema_table_data.fields.length; i++) {
    ...
    if (
      ...
      if (field.references.collection === null) {
        throw new Error(
          `failed to create foreign reference ${field.name} in table ${updated_schema_table_data.collection}`
        );
      }
      foriegn_keys.push({
 ...
 * @returns
 */
const generate_drop_table_query = (table_name: string) => {
...
export const get_field_changes = (
  ...
  type: {
    has_changed: boolean;
    old_value: EntitySchemaRegistry.FieldDefinition["type"];
    new_value: EntitySchemaRegistry.FieldDefinition["type"];
  };
  ...
  const result = {
    ...
    type: {
      has_changed: false,
      old_value: "varchar" as EntitySchemaRegistry.FieldDefinition["type"],
      new_value: "varchar" as EntitySchemaRegistry.FieldDefinition["type"],
    },
  ...
  const current_field = current_schema_fields.find(
    (item) => item.name === existing_field_name
  );
...
export const run = (params: { rootdir: string }) => {
  ...
  try {
    ...
    fs.writeFileSync(
      `${params.rootdir}/migrations/up/${migration_v}.sql`,
      up_queries.join("\n\n"),
      "utf8"
    );
    fs.writeFileSync(
      `${params.rootdir}/migrations/down/${migration_v}.sql`,
      down_queries.join("\n\n"),
      "utf8"
    );
  } catch (error) {
