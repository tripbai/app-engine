...
const create_umzug = (client: UmzugDatabaseClient, cwd: string) => {
  ...
  return new Umzug({
    migrations: {
      glob: pattern,
      cwd,
      resolve: ({ name, path: uppath, context }) => {
    ...
    context: client,
    storage: new JSONStorage({ path: path.join(cwd, 'migrations/.umzug-log.json') }),
    logger: console
  })
}
...
export const run = async (params: {
  ...
  try {
    ...
    require('dotenv').config({ path: get_env_path(environment, params.rootdir) })
    ...
    Application.root(params.rootdir)

    const mySqlPoolService = new MySqlPoolService(
      new MySqlEnvConfig
    )
    ...
    switch (action) {
      ...
      case 'up': 
        ...
        console.log('Running all discovered migrations:', allMigrations.map(m => m.name))
        await uzmug.up()
        break;
      case 'down': 
        await uzmug.down()
