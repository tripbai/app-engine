...
export class AmazonS3StorageService implements AbstractObjectStorageProvider {
  ...
  async storeBuffer(file: Buffer, collection: string, filePath: Core.File.UploadPath){
    const finalPath = `${collection}/${filePath}`
    await this.amazonS3Service.uploadObjectAsBuffer(
      finalPath, file
    )
  }

  async storeText(text: string, collection: string, filePath: Core.File.UploadPath) {
    const finalPath = `${collection}/${filePath}`
    await this.amazonS3Service.uploadObjectAsText(
      finalPath, text
    ) 
  }

  async getObject(collection: string, filePath: Core.File.UploadPath) {
    const finalPath = `${collection}/${filePath}`
    await this.amazonS3Service.getObject(finalPath) 
  }

  async deleteObject(collection: string, filePath: Core.File.UploadPath) {
    const finalPath = `${collection}/${filePath}`
