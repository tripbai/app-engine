...
@injectable()
export class AmazonSNSTopicPublisherService implements AbstractTopicPublisherProvider {
  ...
  async publishTopic(
    ...
    if (topicArn.endsWith('.fifo')) {
      ...
      params.MessageDeduplicationId = options.messageDeduplicationId || Date.now().toString()
