...
import { logInfo } from "../../../application/appLogger";
...
export class FirestoreHelper {
  createOrGetClient(projectId: string, configPath: string) {
    if (!(projectId in FirestoreClientsPool)) {
      ...
      logInfo(`Firestore client created for project with id: ${configPath}`);
    }
  ...
  parseTransactionStep(query: string) {
    ...
    if (tokens[1] === undefined || tokens[1] === null) {
      throw new Error(
        "invalid Firestore single database transaction format: " + query
      );
    }
    return {
