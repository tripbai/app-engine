...
export class FirestoreService implements AbstractDatabaseProvider {
  ...
  constructor(
    ...
    @inject(FirebaseEnvBasedProjectService) public readonly FirebaseProject: FirebaseProjectInterface
  ){}
  ...
  async beginTransaction(transactionSteps: DatabaseTransactionStep[]): Promise<void> {
    try {
      const batch = this.client.batch()
      for (let i = 0; i < transactionSteps.length; i++) {
        const transactionStep = transactionSteps[i]
        const transaction = this.FirestoreHelper.parseTransactionStep(transactionStep.query)
        const entityId = transactionStep.data.entity_id as string
        const documentRef = this.client.collection(transaction.collectionName).doc(entityId)
  ...
  async whereFieldHasValue(
    ...
    try {
      const documentQuery = this.client.collection(collectionName).where(fieldName,'==', fieldValue)
  ...
  async getRecordById(collection: string, entityId: Core.Entity.Id): Promise<FlatDatabaseRecord[]> {
    try {
  ...
  async getRecordsByIds(collection: string, entityIds: Core.Entity.Id[]): Promise<FlatDatabaseRecord[]> {
    try {
