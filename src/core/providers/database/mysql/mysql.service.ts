...
import {
  FatalMySqlConnectionError,
  GenericMySqlException,
  MySqlConnectionError,
} from "./mysql-exceptions";
...
export class MySqlService implements AbstractDatabaseProvider {
  ...
  createRecord(
    ...
    const insertTransactionStep =
      this.MySqlTransactionHelper.generateInsertTransactionStep(
        this,
        collection,
        record
      );
    ...
    if (Array.isArray(insertTransactionStep.data)) {
      throw new LogicException({
        message:
          "mysql insert transaction step should not return data in array type",
  ...
  private async executeTransaction(
    transactionSteps: Array<Readonly<DatabaseTransactionStep>>
  ): Promise<void> {
    /** First, we'll retrieve connection from the Pool */
    const Pool = await this.MySqlPoolService.createOrGetPool();

    const connection = await new Promise<MySqlPoolConnection>(
      (resolve, reject) => {
        Pool.getConnection((error, connection) => {
          if (error) {
            reject(
              new FatalMySqlConnectionError({
    ...
    try {
      ...
      for (let index = 0; index < transactionSteps.length; index++) {
        ...
        const { has_error } = await this.executeTransactionStep(
          connection,
          singleOp
        );
        if (has_error) {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (isDeadlock && attempt < maxRetries) {
          logError({
            severity: 3,
            message: `Deadlock detected, retrying transaction (attempt ${
              attempt + 1
            })...`,
            data: { error },
          });
  ...
  async useSingleQuery(
    ...
    const results = await new Promise<Array<{ [key: string]: any }>>(
      (resolve, reject) => {
        Pool.query(query, data, (error, results, fields) => {
          if (error) {
            reject(
              new MySqlConnectionError({
  ...
  updateRecord(
    ...
    const updateTransactionStep =
      this.MySqlTransactionHelper.generateUpdateTransactionStep(
        this,
        collection,
        record
      );
  ...
  async useQuery(
    transactionableAction: MySqlTransactionStep
  ): Promise<{ [key: string]: any }[]> {
    return await this.useSingleQuery(
      transactionableAction.query,
      transactionableAction.data
    );
  }
}
