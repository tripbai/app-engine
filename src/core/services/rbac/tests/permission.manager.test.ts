...
class TokenAsserterTest {
  isConcrete(
    token: string
  ): asserts token is Core.Authorization.ConcreteToken {}
  isAbstract(
    token: string
  ): asserts token is Core.Authorization.AbstractToken {}
}

const PermissionManager = new PermissionManagerService(new TokenAsserterTest());
...
describe("Permissions Manager", () => {
  describe("populate()", () => {
    ...
    it("should successfully populate token given the placeholders and type of token is correct", () => {
      const abstractPermission =
        "users:{entity_id}:moderate:{post_id}" as Core.Authorization.AbstractToken;
      const concretePermission = PermissionManager.populate(
        abstractPermission,
        { entity_id: "ja62hakshhas", post_id: "ialskahdn213" }
      );
      expect(concretePermission).to.equal(
        "users:ja62hakshhas:moderate:ialskahdn213"
      );
    });
  });
  describe("satisfies()", () => {
    ...
    it("should return true if assigned permission contains a wildcard that matches the required permission", () => {
      const assigned = [
        "posts:*:description:update" as Core.Authorization.ConcreteToken,
      ];
      const required =
        "posts:6158771:description:update" as Core.Authorization.ConcreteToken;
      expect(PermissionManager.satisfies(assigned, required)).to.equal(true);
    ...
    it("should return false if wildcard in assigned permission is way too specific than the required", () => {
      const assigned = [
        "post:61587712:*:update" as Core.Authorization.ConcreteToken,
      ];
