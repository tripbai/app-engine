...
export class UserAccessRegistry extends RegistryRepository<TeamModel> {
   ...
   * owned by the user
   */
  async getOwnedTenantIdOfUserId(
    userId: Core.Entity.Id
  ): Promise<Core.Entity.Id | null> {
   ...
   */
  async getTenantIdsWhereUserIdHasAccessTo(
    userId: Core.Entity.Id
  ): Promise<Array<Core.Entity.Id>> {
  ...
  async getTeamModelOfUserAccessToTenant({
    userId,
    tenantId,
  }: {
    userId: Core.Entity.Id;
