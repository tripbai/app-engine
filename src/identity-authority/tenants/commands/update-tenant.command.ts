...
export class UpdateTenantCommand {
  constructor(
    ...
    @inject(TenantUpdateService)
    private readonly tenantUpdateService: TenantUpdateService,
    @inject(AbstractEventManagerProvider)
    private readonly abstractEventManagerProvider: AbstractEventManagerProvider
  ) {}
  ...
  async execute(
    ...
    try {
      ...
      throw new ResourceAccessForbiddenException({
        ...
        data: {
          requester: requesterAuth,
          tenant_id: tenantModelToUpdate.entity_id,
        },
      });
    }
    if (params.name) {
      await this.tenantUpdateService.updateTenantName(
        tenantModelToUpdate,
        params.name
      );
    }
