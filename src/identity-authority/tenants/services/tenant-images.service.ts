...
export class TenantImagesService {
  ...
  generateUploadToken(
    ...
    return this.iAuthImageTokenService.generate(
      type, tenantId, imagePath
    )
  }
  ...
  parseUploadToken(
    ...
    const tokenPayload = this.iAuthImageTokenService.parse(tenantId, uploadToken)
    if (tokenPayload.type !== 'profile_photo' && tokenPayload.type !== 'cover_photo') {
    ...
    if (tokenPayload.upload_for_entity_id !== tenantId) {
      throw new ResourceAccessForbiddenException({
        ...
        data: { tenant_id: tenantId, user_id: tokenPayload.upload_for_entity_id }
      })
    }
    return {
