...
import {
  getAnthonyFromFireAuthUserModel,
  getJohnUserModel,
  getKyleProfileModel,
  getKyleUserModel,
  getLucyFromGoogleProfileModel,
  getLucyFromGoogleUserModel,
  getMiguelProfileModel,
  getMiguelUserModel,
} from "./dummy-users";
...
const seedDatabase = async () => {
  ...
  const suspendedUserEntityId = createEntityId();
  ...
  const activeUserEntityId = createEntityId();
  ...
  const deactivatedUserEntityId = createEntityId();
  ...
  it("should return access_type=prohibited for suspended user", async () => {
    ...
    try {
      const report = await userAccessReportService.createAccessReport({
        provider: "iauth",
        email_address:
          suspendedUserEmail as IdentityAuthority.Users.Fields.EmailAddress,
        password:
          "helloworld143!" as IdentityAuthority.Users.Fields.RawPassword,
      });
  ...
  it("should return access_type=limited for deactivated user", async () => {
    ...
    try {
      const report = await userAccessReportService.createAccessReport({
        provider: "iauth",
        email_address:
          deactivatedUserEmail as IdentityAuthority.Users.Fields.EmailAddress,
        password:
          "helloworld143!" as IdentityAuthority.Users.Fields.RawPassword,
      });
      expect(report).to.deep.include({
  ...
  it("should return access_type=allowed for active user", async () => {
    ...
    try {
      const report = await userAccessReportService.createAccessReport({
        provider: "iauth",
        email_address:
          activeUserEmail as IdentityAuthority.Users.Fields.EmailAddress,
        password:
          "helloworld143!" as IdentityAuthority.Users.Fields.RawPassword,
      });
      expect(report).to.deep.include({
