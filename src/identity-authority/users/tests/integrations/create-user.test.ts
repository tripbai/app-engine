...
describe('Create User Integration', async () => {
  ...
  it('should not create when email address already exists', async () => {
    ...
    try {
      ...
      await createUserCommand.execute({
        ...
        // @ts-expect-error
        email_address: existingEmail as IdentityAuthority.Users.Fields.EmailAddress
      })
      throw new Error('the above did not throw an error')
    } catch (error) {
  ...
  it('should not create when username already exists', async () => {
    ...
    try {
      ...
      await createUserCommand.execute({
        ...
        status: 'unverified',
        password: 'helloworld143!' as IdentityAuthority.Users.Fields.RawPassword,
        email_address: 'new-email-address@example.com' as IdentityAuthority.Users.Fields.EmailAddress,
        // @ts-expect-error
  ...
  it('should successfully create user', async () => {
    await createUserCommand.execute({
      ...
      status: 'unverified',
      password: 'helloworld143!' as IdentityAuthority.Users.Fields.RawPassword,
      email_address: 'ivanjade1995@example.com' as IdentityAuthority.Users.Fields.EmailAddress,
    ...
    const database = container.get(AbstractDatabaseProvider)
    const record = await database.whereFieldHasValue('users', 'email_address', 'ivanjade1995@example.com')
