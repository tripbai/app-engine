...
export class UserAccessReportService {
  ...
  async createAccessReport(
    params: Omit<
      IdentityAuthority.Users.Endpoints.AccessReport["request"]["data"],
      "provider"
    > & {
    ...
    if (userModel.identity_provider === "iauth") {
      ...
      throw new LogicException({
        message:
          "there may be identity provider that is not yet supported here",
    ...
    return this.generateStatusBasedReport(userModel, accessToken);
  }
  ...
  generateStatusBasedReport(
    ...
    if (status === "banned" || status === "suspended") {
      const prohibited: IdentityAuthority.Users.ApplicationAccess.Prohibited =
        status;
      return {
