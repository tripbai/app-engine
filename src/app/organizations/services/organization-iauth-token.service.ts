...
export class OrganizationIAuthTokenService {
  ...
  async parseToken(token: string) {
    const payload = this.jwtProvider.parse(
      AppENV.get('SECRET_KEY'), token
    )
    const expectedIssuer: IdentityAuthority.Tenants.CertificationTokenData.Issuer = 'identity-authority:tenant-access-certifier'
    ...
    if (typeof payload.data.tenant_id !== 'string' || payload.data.tenant_id === '') {
      throw new Error('Invalid tenant ID in token data')
    }
    EntityToolkit.Assert.idIsValid(payload.data.tenant_id)
    if (!('user_id' in payload.data)) {
      throw new Error('Missing user ID in token data')
    }
    if (typeof payload.data.user_id !== 'string' || payload.data.user_id === '') {
      throw new Error('Invalid user ID in token data')
    }
    EntityToolkit.Assert.idIsValid(payload.data.user_id)
