...
export class OrganizationAuthService {
  constructor(
    ...
    @inject(UserStoreAccessRegistry)
    private userStoreAccessRegistry: UserStoreAccessRegistry,
  ...
  async exchangeIAuthTokenToOrganizationToken(params: {
    ...
    const payload = await this.organizationIAuthTokenService.parseToken(
      params.iAuthCertificationToken
    );
  ...
  handleOwnerAccess(params: {
    ...
    const permissions = this.makeOrgOwnerAdminPermissionsForAllStores(
      params.tenantId
    );
    return this.requesterTokenService.generate({
  ...
  makeOrgOwnerAdminPermissionsForAllStores(organizationId: Core.Entity.Id) {
    return this.organizationPermissionService.createOrganizationLevelPermission(
      organizationId
    );
  }
  ...
  async filterStoreIdsUserHaveAccessTo(params: {
    ...
    const allStoresUserHaveAccessTo =
      await this.userStoreAccessRegistry.getStoreIdsUserHasAccess(
        params.userId
      );
