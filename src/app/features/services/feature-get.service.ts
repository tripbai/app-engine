...
export class FeatureGetService {
  constructor(
    ...
    private defaultFeaturesRepository: DefaultFeaturesRepository,
    @inject(FeaturesOverrideRepository)
    private featuresOverrideRepository: FeaturesOverrideRepository
  ) {}
  ...
  async getFeatureByKey<T extends keyof FeaturesList>(params: {
    ...
    const [defaultFeature, overrideFeature] = await Promise.all([
      this.defaultFeaturesRepository
        .getFeatureKey(params.featureKey)
        .byPackageId(params.packageId),
      this.featuresOverrideRepository
        .getFeatureKeyOfPackage(params.featureKey, params.packageId)
        .ownedByEntityId(params.ownerId),
    ]);
