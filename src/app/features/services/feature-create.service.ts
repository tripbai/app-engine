...
import { BadRequestException, RecordAlreadyExistsException, RecordNotFoundException, ResourceAccessForbiddenException } from "../../../core/exceptions/exceptions";
...
export class FeatureCreateService {
  ...
  constructor(
    @inject(DefaultFeaturesRepository) private readonly defaultFeaturesRepository: DefaultFeaturesRepository,
    @inject(FeaturesOverrideRepository) private readonly featureOverrideRepository: FeaturesOverrideRepository
  ) {}
  ...
  async registerDefaultFeatureInPackage(params: {
    ...
    if (!params.organizationRequester.isWebAdmin()) {
      throw new ResourceAccessForbiddenException({
        ...
        data: {package_id: params.packageModel.entity_id, feature_key: params.featureKey, feature_value: params.featureValue}
      })
    }
    ...
    const newFeatureId = Pseudorandom.alphanum32()
    await this.isFeatureExistsInPackage(
      params.featureKey,
      params.packageModel
    )
  ...
  async createFeatureOverride(params: {
    organizationRequester: OrganizationRequester
    ...
    if (defaultFeature === null) {
      throw new RecordNotFoundException({
        ...
        data: {package_id: params.packageModel.entity_id, feature_key: params.featureKey}
      })
    }
    ...
    if (!defaultFeature.org_mutable && params.organizationRequester.isWebAdmin() === false) {
      throw new ResourceAccessForbiddenException({
    ...
    if (params.featurableEntityType === 'organization') {
      ...
      if (!params.organizationRequester.isOrganizationAdminOf(featurableEntityId)) {
        throw new ResourceAccessForbiddenException({
          message: 'only organization admin can register feature override for organization',
          data: {organization_id: featurableEntityId, feature_key: params.featureKey}
        })
      }
    }
    ...
    if (params.featurableEntityType === 'store') {
      ...
      if (!params.organizationRequester.canOperateStore({
        ...
        throw new ResourceAccessForbiddenException({
          message: 'only user with store access can register feature override for store',
          data: {store_id: featurableEntityId, feature_key: params.featureKey}
        })
      }
    }
    ...
    await this.featureOverrideRepository.createFeatureOverride(
      params.featureKey, defaultFeature
    )
  }
  ...
  async isFeatureAlreadyOverriden(
    ...
    const existingFeatureOverride 
      = await this.featureOverrideRepository
  ...
  async isFeatureExistsInPackage(
    ...
    const exitingFeature 
      = await this.defaultFeaturesRepository
