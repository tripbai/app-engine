...
export class ToursScheduleGetController {
  ...
  constructor(
    @inject(GetToursScheduleQuery) public readonly getToursScheduleQuery: GetToursScheduleQuery
  ) {}

  @get<TripBai.Schedules.Endpoints.GetToursSchedule>('/tripbai/schedules/tours?store_id=:store_id&from_date=:from_date&range=:range')
  async getToursSchedule<T extends TripBai.Schedules.Endpoints.GetToursSchedule>(
    params: Core.Route.ControllerDTO<T>
  ): Promise<T['response']> {
    const commandDTO: Parameters<GetToursScheduleQuery["execute"]>[0] = Object.create(null)
